...
- (UIView *)view
{
    if(self.videoArry != nil){
        [self.videoArry removeAllObjects];
    }else{
        self.videoArry = [[NSMutableArray alloc]init];
    }
...
RCT_EXPORT_METHOD(finishCapture :(RCTPromiseResolveBlock)resolve :(RCTPromiseRejectBlock)reject) {
    if(self.videoArry != nil&&
       self.videoArry.count>0){
        //视频合成
        [self mergeVideoToOneVideo:self.videoArry :resolve :reject];
    }
}

//iOS录制视频格式是.mov格式，所以需要将mov格式转化为mp4格式
- (void)transformVideo:(NSString *)filePath byQuality:(NSString*)compressQuality
{
    @try {
        ...
        [exportSession exportAsynchronouslyWithCompletionHandler:^(void) {
            ...
            {
                NSLog(@"生成的视频路径%@",outputFilePath);
                if(self.videoArry != nil){
                    [self.videoArry addObject:outputFilePath];
                }
            }
            else
            {
                NSLog(@"Cannot compress video");
            }
        }];
    } @catch(NSException *e) {
        NSLog(@"%@",e.reason);
 ...
 *
 */
- (void)mergeVideoToOneVideo:(NSMutableArray *)videoArry :(RCTPromiseResolveBlock)resolve :(RCTPromiseRejectBlock)reject
{
    AVMutableComposition *mixComposition = [self mergeVideos:videoArry :reject];
 ...
 */
-(AVMutableComposition *)mergeVideos:(NSMutableArray*)array :(RCTPromiseRejectBlock)reject
{
                ...
                error:(NSError *)error
{
  ...
  if (self.videoTarget == RCTCameraCaptureTargetCameraRoll) {
    ...
    self.videoResolve(videoInfo);
      //将录制的mov格式视频转化为mp4格式
      [self transformVideo:fullPath byQuality:@"medium"];
  }
  else {
